  ...
  return (
    <Card className="overflow-hidden border-2">
      <div className="relative" style={{ height: 520 }}>
        ...
        {/* Kanan atas: Koordinat + Scale + Legenda */}
            ...
            {dataTerfilter.map((fitur, idx) => {
              ...
              return (
                <tr key={d.nama_provinsi}
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold border"
                      style={{ borderColor: w + '60', color: w, backgroundColor: w + '15' }}>
                      {kat}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-center">
...
export default function KesehatanPage() {
  ...
  const refreshDB = async () => {
    try {
      const r = await axios.get(`${API}/health-analysis/list/`);
  ...
  const handleSearch = (nama) => {
    ...
    const f = hasilAnalisis?.matched_features?.features?.find(
      feat => feat.properties?.health_analysis?.nama_provinsi?.toLowerCase() === n.toLowerCase()
    );
  ...
  const eksporData = (format) => {
    ...
    if (format === 'EXCEL') {
      const ws = XLSX.utils.json_to_sheet(r.map(i => ({
        Provinsi:        i.provinsi,
        Kategori:        i.kategori,
        'Skor Total':    i.skor_total,
        'AHH (tahun)':   i.ahh       || '-',
        'Imunisasi (%)': i.imunisasi  || '-',
        'Sanitasi (%)':  i.sanitasi   || '-',
      })));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Analisis');
      XLSX.writeFile(wb, `Analisis_Kesehatan_BPS_${thn}_${tgl}.xlsx`);
                  ...
                  <button key={th} onClick={() => setTahunTerpilih(th)}
                    className={cn('px-3 py-2.5 rounded-xl text-sm font-bold border-2 transition-all',
                      tahunTerpilih === th
                        ...
                        : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-emerald-300 dark:hover:border-emerald-600')}>
                    {th}
                  </button>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md p-8">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-emerald-500 outline-none text-sm mb-6"
            />
            <div className="flex gap-3">
