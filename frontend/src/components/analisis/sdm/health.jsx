          ...
          {sedangCek
            ? keysLoading.map(k => (
                <div key={k} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-slate-50 dark:bg-slate-800">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400 flex-1">{DATASET_LABELS_KESEHATAN[k]}</span>
        ...
        {!sedangCek && (
          <div className="px-6 pb-5 flex gap-3">
            <button onClick={onTutup}
              className="flex-1 px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl font-semibold text-sm transition-colors">
        ...
        <div className="px-6 py-5 bg-amber-50 dark:bg-amber-900/20 flex items-center gap-3 border-b border-amber-100 dark:border-amber-800/30">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">{INDIKATOR_LABELS_KESEHATAN[indikator]}</span>
...
export function SelectorAnalisisKesehatan({ hasilAnalisis, kombinasiTersedia, tahunTerpilih, onPilih, sedangMuatAwal }) {
  ...
  // Shared class untuk item dropdown — konsisten light & dark
    ...
    <div ref={wrapRef} className="flex items-stretch shadow-md rounded-lg">
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-2.5 h-10 px-4 min-w-[200px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-l-lg text-sm font-medium text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors select-none`}>
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-2.5 h-10 px-4 min-w-[130px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 border-l-0 rounded-r-lg text-sm font-medium text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors select-none`}>
...
export function TrendPanelKesehatan({ daftarTersimpan, onTutup }) {
  ...
  const DeltaBadge = ({ val, positif = true }) => {
    ...
    return (
      <span className={`flex items-center gap-0.5 text-[9px] font-bold ${good ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700">
                {chartData.length === 0 ? (
                  <div className="h-48 flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm">
                    Tidak ada data untuk indikator ini
                  </div>
                ) : (
                  <>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <div className="text-[10px] text-slate-500 dark:text-slate-400">
                          {INDIKATOR_LABELS_KESEHATAN[filterInd]} · {chartData.length} titik data
                        </div>
                      </div>
                        ...
                        <table className="w-full text-left min-w-[400px]">
                          <thead className="sticky top-0 bg-slate-100 dark:bg-slate-700">
                            <tr className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">
                              {['Tahun','Indikator','STABIL','WASPADA','KRITIS','Total'].map(h => (
                              ...
                              <tr key={i} className="hover:bg-white dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-3 py-2">
                                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                                    style={{ background: INDIKATOR_COLORS_KESEHATAN[filterInd]+'20', color: INDIKATOR_COLORS_KESEHATAN[filterInd] }}>
                                    {filterInd}
                                  </span>
                                </td>
                                <td className="px-3 py-2 text-xs font-bold text-green-600 dark:text-green-400">{row.STABIL}</td>
                                <td className="px-3 py-2 text-xs font-bold text-amber-600 dark:text-amber-400">{row.WASPADA}</td>
              ...
              <div className="flex flex-col gap-3">
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700">
                  <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">
                    % Provinsi STABIL (Terbaru)
                  </div>
                      ...
                      return (
                        <div key={ind}>
                          ...
                          <div className="flex justify-between text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">
                            <span>{last.STABIL} STABIL</span><span>{last.KRITIS} KRITIS</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {delta && (
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl p-4 border border-emerald-200 dark:border-emerald-800">
                    <div className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider mb-2.5">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {TAHUN_TERSEDIA.map(thn => {
                      ...
                      return (
                        <div key={thn} className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold border ${
                          ada
            ...
            {chartData.length >= 2 && (
              <div className="mt-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700">
                <div className="text-xs font-bold text-slate-900 dark:text-white mb-0.5">
                  Total Provinsi Teranalisis per Tahun
                </div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 mb-3">
                  {INDIKATOR_LABELS_KESEHATAN[filterInd]}
                </div>
                <ResponsiveContainer width="100%" height={90}>
                  <LineChart data={chartData} margin={{ top:4, right:16, left:-20, bottom:0 }}>
                    ...
                    <Tooltip content={<CustomTooltip />} />
                    <Line type="monotone" dataKey="TOTAL" name="Total Provinsi"
                      stroke={INDIKATOR_COLORS_KESEHATAN[filterInd]} strokeWidth={2.5}
      ...
      <div className="p-6 space-y-5">
              ...
              {menuDatasetTerbuka && (
                <div className="absolute top-full mt-1 right-0 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-2xl z-[1002] overflow-hidden border border-slate-200 dark:border-slate-700 py-1">
                  {[
                    ...
                    <button key={k} onClick={() => onUnduhDataset(k)}
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
          <div className="text-xs font-bold text-slate-700 dark:text-slate-200 mb-2 flex items-center gap-1.5">
            <Database size={13} className="text-slate-500"/> Sumber Data — BPS Web API Tahun {hasilAnalisis?.tahun || tahunTerpilih}
          </div>
          <div className="space-y-1.5">
            {(ind === 'ALL' || ind === 'AHH') && (
              <div className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-red-400 mt-1.5 flex-shrink-0"/>
                <span><strong>AHH</strong> — BPS Susenas, Variabel 501. Angka harapan hidup saat lahir (laki-laki & perempuan) per provinsi, dirata-rata.</span>
              </div>
            )}
            {(ind === 'ALL' || ind === 'IMUNISASI') && (
              <div className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 flex-shrink-0"/>
                <span><strong>Imunisasi</strong> — BPS Susenas, Variabel 2280. Persentase balita imunisasi dasar lengkap per provinsi.</span>
              </div>
            )}
            {(ind === 'ALL' || ind === 'SANITASI') && (
              <div className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-400 mt-1.5 flex-shrink-0"/>
                <span><strong>Sanitasi</strong> — BPS, Variabel 847. Persentase rumah tangga dengan akses sanitasi layak per provinsi.</span>
              </div>
            )}
          </div>
        </div>
        ...
        {ind === 'ALL' && (
          <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800">
            <div className="text-xs font-bold text-emerald-800 dark:text-emerald-200 mb-3 flex items-center gap-1.5">
              <BarChart2 size={13}/> Mengapa Bobot 40% · 35% · 25%?
            </div>
            ...
            <div className="mt-3 p-3 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
              <p className="text-[11px] text-emerald-700 dark:text-emerald-300 font-mono font-semibold">
                IKK = (AHH × 0.40) + (Imunisasi × 0.35) + (Sanitasi × 0.25)
              </p>
              <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1">
                Skor tiap indikator: 3=STABIL · 2=WASPADA · 1=KRITIS &nbsp;|&nbsp; STABIL ≥ 2.4 · WASPADA ≥ 1.8 · KRITIS &lt; 1.8
              </p>
            </div>
          </div>
        )}
            ...
            <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 border-l-4 border-red-500">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h5 className="text-sm font-bold text-red-900 dark:text-red-100">Angka Harapan Hidup (AHH)</h5>
                </div>
                <span className="text-[10px] px-2 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded font-semibold">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-lg p-3 mb-3 border border-red-200 dark:border-red-700">
                <div className="text-[10px] font-bold text-red-600 dark:text-red-400 mb-1.5">📐 CARA HITUNG</div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border-l-4 border-blue-500">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h5 className="text-sm font-bold text-blue-900 dark:text-blue-100">Cakupan Imunisasi Dasar Lengkap</h5>
                </div>
                <span className="text-[10px] px-2 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded font-semibold">
                  {ind === 'ALL' ? 'Bobot 35%' : 'Indikator Tunggal'}
                </span>
              </div>
            ...
            <div className="bg-teal-50 dark:bg-teal-900/20 rounded-xl p-4 border-l-4 border-teal-500">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h5 className="text-sm font-bold text-teal-900 dark:text-teal-100">Akses Sanitasi Layak</h5>
                </div>
                <span className="text-[10px] px-2 py-0.5 bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300 rounded font-semibold">
                  {ind === 'ALL' ? 'Bobot 25%' : 'Indikator Tunggal'}
                </span>
              </div>
              <div className="bg-white dark:bg-slate-900 rounded-lg p-3 mb-3 border border-teal-200 dark:border-teal-700">
                <div className="text-[10px] font-bold text-teal-600 dark:text-teal-400 mb-1.5">📐 CARA HITUNG</div>
                <ol className="space-y-1 text-[11px] text-slate-600 dark:text-slate-300">
