...
import {
  Download, ChevronDown, Info, Calendar,
...
function TabInfo({ hasilAnalisis,jumlahKategori,indikatorTerpilih,kategoriTerpilih,setKategoriTerpilih,eksporData,getWarna,getKategori }) {
  ...
  if(!hasilAnalisis) return (
    <div className="py-16 text-center">
      ...
      <p className="text-base text-slate-500 dark:text-slate-400">Belum ada data. Klik <strong>Analisis IPE</strong> di peta.</p>
    </div>
  );
...
function TabMetodologi() {
  return (
    <div className="space-y-3">
      ...
      <MetSection accentColor="#6366f1" title="Konsep Dasar IPE" sub="Indeks komposit kinerja pertumbuhan ekonomi provinsi, skala 0–100" defaultOpen>
        <div className="space-y-3">
          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
            IPE (Indeks Pertumbuhan Ekonomi) adalah indeks komposit terstandarisasi yang mengukur kinerja pertumbuhan ekonomi wilayah provinsi pada skala <strong className="text-slate-800 dark:text-white">0 – 100</strong>. IPE dibentuk dari dua indikator resmi BPS yang saling melengkapi:
          </p>
            ...
            {[
              {
                icon:'📈',
                color:'#10b981',
                label:'Dimensi 1 — Dinamika',
                nama:'Laju Pertumbuhan PDRB ADHK (%)',
                desc:'Mengukur kecepatan tumbuh ekonomi riil (flow). Basis ADHK 2010 mengeliminasi pengaruh inflasi.',
                bobot:'50%',
                skor:'S1',
              },
              {
                icon:'💰',
                color:'#f59e0b',
                label:'Dimensi 2 — Kapasitas',
                nama:'PDRB per Kapita ADHK (Ribu Rp)',
                desc:'Mengukur output ekonomi per penduduk (stock proxy). Mencerminkan kemampuan produktif wilayah.',
                bobot:'50%',
                skor:'S2',
              },
            ].map(d=>(
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Langkah 1a — Normalisasi Laju PE (S1)</p>
            </div>
            <MathBlock>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                ['g','Laju PE (%) provinsi aktual','Langsung dari tabel BPS tahun bersangkutan'],
                ['g_min','Nilai TERENDAH seluruh 38 prov','Ambil MIN dari semua baris provinsi di tabel BPS tahun tersebut'],
                ['g_max','Nilai TERTINGGI seluruh 38 prov','Ambil MAX dari semua baris provinsi di tabel BPS tahun tersebut'],
              ].map(([p,v,d])=>(
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Langkah 1b — Normalisasi PDRB/Kapita (S2)</p>
            </div>
            <MathBlock>
              <div className="flex items-center gap-1 flex-wrap">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                ['y','PDRB/Kapita (Ribu Rp) provinsi','Data dari tabel BPS, kolom PDRB per kapita ADHK 2010'],
                ['y_min','Nilai TERENDAH seluruh 38 prov','MIN seluruh baris provinsi di tabel BPS tahun tersebut'],
                ['y_max','Nilai TERTINGGI seluruh 38 prov','MAX seluruh baris provinsi di tabel BPS tahun tersebut'],
              ].map(([p,v,d])=>(
                <div key={p} className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-600">
                  <div className="text-[10px] text-slate-400 font-mono mb-0.5">{p}</div>
                  <div className="text-xs font-bold text-slate-800 dark:text-slate-100">{v}</div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Langkah 2 — Hitung IPE (Equal Weight 50:50)</p>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              {[
                ['S1','Skor ternormalisasi Laju PE (0–100)'],
                ['S2','Skor ternormalisasi PDRB/Kapita (0–100)'],
                ['0,50','Bobot equal weight (tidak ada rujukan resmi yang berbeda)'],
                ['IPE','Indeks Pertumbuhan Ekonomi provinsi, skala 0–100'],
              ].map(([v,d])=>(
                <div key={v} className="flex items-start gap-2 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-600">
                  <span className="font-mono font-black text-indigo-600 dark:text-indigo-400 text-sm flex-shrink-0 w-8">{v}</span>
          ...
          {/* Contoh */}
          <div>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Contoh Konkret (Ilustrasi)</p>
            <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-600">
              <table className="w-full text-xs">
                <thead className="bg-slate-100 dark:bg-slate-700">
                  <tr className="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                    ...
                    <th className="px-3 py-2 text-center">Laju PE (%)</th>
                    <th className="px-3 py-2 text-center">PDRB/Kap (rb Rp)</th>
                    <th className="px-3 py-2 text-center">S1</th>
                    <th className="px-3 py-2 text-center">S2</th>
                    <th className="px-3 py-2 text-center">IPE</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {[
                    {n:'Maluku Utara',g:'22,94',y:'28.500',s1:'100,0',s2:'74,1',ipe:'87,1',cat:'SANGAT TINGGI',w:'#008cd6'},
                    {n:'Jawa Tengah', g:'5,24', y:'32.100',s1:'17,4', s2:'100,0',ipe:'58,7',cat:'TINGGI',w:'#abcd05'},
                    {n:'Papua',       g:'1,50', y:'18.200',s1:'0,0',  s2:'0,0',  ipe:'0,0', cat:'RENDAH',w:'#af4284'},
                  ].map((r,i)=>{
                    const isDark=['#fff67f','#abcd05'].includes(r.w);
                    return (
                      <tr key={i} className={i%2===0?'bg-white dark:bg-slate-800':'bg-slate-50/40 dark:bg-slate-800/60'}>
                        <td className="px-3 py-2 font-semibold text-slate-800 dark:text-slate-100">{r.n}</td>
                        <td className="px-3 py-2 text-center text-emerald-600 dark:text-emerald-400 font-semibold">{r.g}</td>
                        <td className="px-3 py-2 text-center text-amber-600 dark:text-amber-400 font-semibold">{r.y}</td>
                        <td className="px-3 py-2 text-center text-emerald-700 dark:text-emerald-300 font-bold">{r.s1}</td>
                        <td className="px-3 py-2 text-center text-amber-700 dark:text-amber-300 font-bold">{r.s2}</td>
                        <td className="px-3 py-2 text-center">
                          <span className="px-2 py-0.5 rounded font-black text-xs" style={{backgroundColor:r.w,color:isDark?'#1a2e00':'#fff'}}>{r.ipe}</span>
                        </td>
                      </tr>
                    );
                  })}
                  <tr className="bg-slate-50 dark:bg-slate-700/40 text-[9px] text-slate-400">
                    <td className="px-3 py-2 italic">g_min / y_min</td>
                    <td className="px-3 py-2 text-center">1,50</td>
                    <td className="px-3 py-2 text-center">18.200</td>
                    <td colSpan={3}/>
                  </tr>
                  <tr className="bg-slate-50 dark:bg-slate-700/40 text-[9px] text-slate-400">
                    <td className="px-3 py-2 italic">g_max / y_max</td>
                    <td className="px-3 py-2 text-center">22,94</td>
                    <td className="px-3 py-2 text-center">32.100</td>
                    <td colSpan={3}/>
                  </tr>
                </tbody>
              </table>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
              Maluku Utara (IPE 87,1) — SANGAT TINGGI karena laju pertumbuhannya tertinggi, meski PDRB/kapita bukan yang terbesar. Jawa Tengah (58,7) — TINGGI karena PDRB/kapita tertinggi. Papua (0,0) — RENDAH karena berada di posisi terendah pada kedua indikator dalam ilustrasi ini.
            </p>
          </div>
        </div>
      </MetSection>
      ...
      {/* KLASIFIKASI */}
      <MetSection accentColor="#10b981" title="Klasifikasi Nilai IPE" sub="4 kelas berdasarkan rentang skor" defaultOpen>
      ...
      <MetSection accentColor="#14b8a6" title="Sumber Data & Link BPS" sub="Dataset resmi yang digunakan">
        <div className="space-y-2.5">
          {[
            {
              col:'laju_pe',
              nama:'Laju Pertumbuhan PDRB ADHK 2010 Menurut Provinsi [Seri 2010]',
              satuan:'Persen (%)',
              k:'S1 (Laju PE)',
            ...
            {
              col:'pdrb_kapita',
              nama:'PDRB per Kapita ADHK 2010 Menurut Provinsi',
              satuan:'Ribu Rupiah / Kapita / Tahun',
              k:'S2 (PDRB/Kap)',
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/40 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Regresi Linear (Ordinary Least Squares) adalah metode statistik untuk mencari hubungan antara variabel waktu (tahun) dengan nilai data. Metode ini bekerja dengan menarik satu <strong>garis lurus paling optimal (best-fit line)</strong> di antara titik-data historis yang ada, lalu memperpanjang garis tersebut ke masa depan untuk membaca arah tren apakah cenderung naik, turun, atau stabil.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              {[
                ['ŷ','Nilai proyeksi (Laju PE / PDRB per Kapita)'],
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                {icon:'📊',title:'Fokus Tren Jangka Panjang',desc:'Sangat stabil dalam memetakan arah pertumbuhan makro (tren naik/turun) hingga tahun 2045 tanpa terganggu oleh fluktuasi jangka pendek yang ekstrem.'},
                {icon:'🛡️',title:'Adaptasi Data Terbatas',desc:'Pendekatan statistik paling aman untuk mencegah overfitting pada data historis yang memiliki keterbatasan jumlah sampel tahunan.'},
              ].map((r,i)=>(
          ...
          {/* Catatan penting: min-max proyeksi */}
          <div className="p-3.5 rounded-xl border border-sky-200 dark:border-sky-700 bg-sky-50 dark:bg-sky-900/20 flex items-start gap-3">
            <AlertCircle size={15} className="text-sky-500 flex-shrink-0 mt-0.5"/>
            <div>
              <p className="text-sm font-bold text-sky-800 dark:text-sky-200 mb-1">Catatan Khusus untuk IPE: Dinamika Min-Max</p>
              <p className="text-xs text-sky-700 dark:text-sky-300 leading-relaxed">
                Karena IPE menggunakan normalisasi Min-Max berbasis seluruh provinsi pada tahun yang sama, nilai min dan max untuk tahun proyeksi (2025–2045) juga merupakan hasil proyeksi OLS. Ini berarti <strong>skor S1 dan S2 bersifat relatif antar provinsi di tahun proyeksi tersebut</strong>, bukan perbandingan lintas tahun. Interpretasi kategori IPE antar tahun yang berbeda perlu dilakukan dengan hati-hati.
              </p>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <ul className="mt-2 space-y-1">
                  {[
                    'Guncangan ekonomi global (krisis, pandemi, perang)',
                    'Perubahan kebijakan fiskal dan investasi besar',
                    'Pergeseran struktur industri daerah',
                    'Bencana alam / perubahan iklim ekstrem',
                    'Lompatan teknologi yang masif (otomasi, AI)',
                  ].map((item,i)=>(
      ...
      <MetSection accentColor="#8b5cf6" title="Justifikasi Metodologi" sub="Mengapa Min-Max dengan bobot equal weight?">
        <div className="space-y-3">
          {[
            {icon:'⚖️',title:'Equal Weight (50:50)',desc:'Tidak ada rujukan resmi yang menetapkan bobot berbeda untuk Laju PE dan PDRB per Kapita. Kedua dimensi dianggap setara pentingnya — laju pertumbuhan (kecepatan) dan kapasitas output (kekuatan) sama-sama krusial dalam mengukur kinerja ekonomi komprehensif.'},
            {icon:'📐',title:'Min-Max Normalisasi',desc:'Dipilih karena menghasilkan skor 0–100 yang intuitif dan mudah diperbandingkan. Berbeda dengan standardisasi Z-score, Min-Max mempertahankan distribusi asli data tanpa asumsi normalitas, cocok untuk data ekonomi wilayah yang sering tidak terdistribusi normal.'},
            {icon:'🏛️',title:'ADHK 2010 sebagai Basis',desc:'Penggunaan harga konstan 2010 mengikuti standar BPS dan SNA 2008, sehingga perbandingan antar tahun bebas dari distorsi inflasi. Angka yang dihasilkan mencerminkan pertumbuhan riil kapasitas produksi wilayah.'},
          ].map((item,i)=>(
            ...
            return(
              <button key={ind} onClick={()=>ada&&setFI(ind)}
                className={cn('px-3 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-1',
                  ...
                  ada?'text-slate-500 hover:text-slate-700 dark:hover:text-slate-200':'text-slate-300 dark:text-slate-600 cursor-not-allowed')}>
...
// ─── MAIN TABS WRAPPER ─────────────────────────────────────────────────────────
export default function TabsIPE({
          ...
          return(
            <button key={id} onClick={()=>setActiveTab(id)}
              className={cn('flex items-center justify-center gap-2 px-5 py-4 text-sm font-semibold transition-all relative flex-1 whitespace-nowrap',
                active?'text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/20':'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/30')}>
