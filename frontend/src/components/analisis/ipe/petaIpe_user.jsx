"use client";
// ─── PETA IPE — USER (Read + Navigate) ───────────────────────────────────────
// Seperti petaIpe tapi tanpa tombol Analisis / Simpan / Reset.
// User hanya bisa navigasi peta, ganti tahun/indikator dari data tersimpan,
// dan filter kategori.
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  BASEMAPS_IPE, PUSAT_DEFAULT_IPE, ZOOM_DEFAULT_IPE, KATEGORI_IPE,
...
function buildTooltipHTML_IPE(a, w, kat, indikatorTerpilih) {
  ...
  const srcBadge = sumber === 'campuran'
    ? `<div style="margin-top:5px;display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px;font-size:9px;font-weight:700;background:#fffbeb;border:1px solid #fcd34d;color:#92400e;">📈 Aktual + Prediksi OLS</div>`
    : sumber === 'prediksi'
...
function SelectorUser_IPE({
  hasilAnalisis, kombinasiTersedia, tahunTerpilih, onPilih,
      ...
      <div className="relative">
        ...
        >
          ...
          <span className={cn('flex-1 text-left', mungkinPrediksi && 'text-amber-600 dark:text-amber-400 font-semibold')}>
            {activeThn}
            {mungkinPrediksi && (
              <span className="ml-1 text-[8px] bg-amber-100 dark:bg-amber-900/60 text-amber-600 px-1 py-0.5 rounded">proj</span>
            )}
          </span>
...
// ─── PETA IPE USER ────────────────────────────────────────────────────────────
export default function PetaIPE_User({
  ...
  const geoProps = hasilAnalisis?.matched_features?.features ? {
    ...
    style: (fitur) => {
      ...
      const w   = getWarna_IPE(fitur, indikatorTerpilih);
