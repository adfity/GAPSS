...
import {
  ...
  BarChart2, Leaf, Activity, Search, TrendingUp,
} from 'lucide-react';

// ─── CONSTANTS ────────────────────────────────────────────────────────────────
export const KATEGORI_ISKA = {
...
export const TAHUN_BPS_AKTUAL_ISKA = Array.from({ length: 16 }, (_, i) => 2010 + i); // 2010–2025
export const TAHUN_OLS_ISKA        = Array.from({ length: 20 }, (_, i) => 2026 + i); // 2026–2045
export const TAHUN_TERSEDIA_ISKA   = [...TAHUN_BPS_AKTUAL_ISKA, ...TAHUN_OLS_ISKA];

export const DATASET_LABELS_ISKA = {
  padi:         'Produktivitas Padi (Ku/Ha)',
  hortikultura: 'Total Produksi Hortikultura (Kw)',
...
export const INDIKATOR_LABELS_ISKA = {
  ALL:           'ISKA Komposit (7 Dimensi)',
  PERTANIAN:     'D1 — Pertanian (Padi)',
  HORTIKULTURA:  'D2 — Hortikultura',
  KEHUTANAN:     'D3 — Kehutanan (Tutupan Lahan)',
  HASIL_HUTAN:   'D4 — Hasil Hutan (Kayu Bulat)',
  PERKEBUNAN:    'D5 — Perkebunan',
  PERIKANAN:     'D6 — Perikanan',
  PERTAMBANGAN:  'D7 — Pertambangan (PDRB)',
};

export const INDIKATOR_ICON_ISKA = {
  ALL:           <BarChart2 size={13} />,
  PERTANIAN:     <Leaf size={13} />,
  HORTIKULTURA:  <Leaf size={13} />,
  KEHUTANAN:     <Leaf size={13} />,
  HASIL_HUTAN:   <Leaf size={13} />,
  PERKEBUNAN:    <Leaf size={13} />,
  PERIKANAN:     <Leaf size={13} />,
  PERTAMBANGAN:  <Activity size={13} />,
};

export const INDIKATOR_COLORS_ISKA = {
  ALL:           '#10b981',
  PERTANIAN:     '#84cc16',
  HORTIKULTURA:  '#22c55e',
  KEHUTANAN:     '#14b8a6',
...
export const PUSAT_DEFAULT_ISKA = [-2.5, 118];
export const ZOOM_DEFAULT_ISKA  = 5;

// ─── HELPERS ─────────────────────────────────────────────────────────────────
export const getWarna_ISKA = (fitur) => {
  const a = fitur?.properties?.iska_analysis || {};
  return a.warna || '#a6a6a6';
};

export const getKategori_ISKA = (fitur) => {
  const a = fitur?.properties?.iska_analysis || {};
...
const DIMENSI_ROWS = [
  { key: 's_padi',         label: 'D1 Pertanian',    satuan: 'Ku/Ha',      rawKey: 'padi' },
  { key: 's_hortikultura', label: 'D2 Hortikultura', satuan: 'Kw',         rawKey: 'hortikultura' },
  { key: 's_ttplahan',     label: 'D3 Kehutanan',    satuan: 'Ha',         rawKey: 'ttplahan' },
  { key: 's_hahutan',      label: 'D4 Hasil Hutan',  satuan: 'M³',         rawKey: 'hahutan' },
  { key: 's_kebun',        label: 'D5 Perkebunan',   satuan: 'Ton',        rawKey: 'kebun' },
  { key: 's_ikan',         label: 'D6 Perikanan',    satuan: 'Ton',        rawKey: 'ikan' },
  { key: 's_proktam',      label: 'D7 Tambang',      satuan: 'Miliar Rp',  rawKey: 'proktam' },
];

// ─── SELECTOR ─────────────────────────────────────────────────────────────────
export function SelectorAnalisis_ISKA({
      ...
      <div className="relative">
        ...
        {openTahun && (
          <div className={cn(dropdownBase, "right-0 min-w-[160px] max-h-72 overflow-y-auto")}>
            <div className="px-3 py-1.5 text-[9px] font-bold text-slate-500 uppercase tracking-wider bg-slate-50 dark:bg-slate-700/60 sticky top-0">Pilih Tahun</div>
            {[...TAHUN_BPS_AKTUAL_ISKA].reverse().map(th => (
            ...
            <div className="px-3 py-1.5 text-[9px] font-bold text-amber-500 uppercase tracking-wider bg-amber-50 dark:bg-amber-900/30 flex items-center gap-1">
              <TrendingUp size={9} /> Prediksi OLS
            </div>
            {TAHUN_OLS_ISKA.map(th => (
              <button key={th} onClick={() => { setOpenTahun(false); onPilih(th, activeInd); }}
                className={cn(itemBase, activeThn === th && itemActive, "justify-between")}>
...
// ─── TOOLTIP HTML ─────────────────────────────────────────────────────────────
function buildTooltipHTML_ISKA(a, w) {
  const skor      = a.skor_dimensi   || {};
  const raw       = a.data_raw       || {};
  ...
  const srcBadge = (sumber === 'campuran' || sumber === 'prediksi')
    ? `<div style="margin-top:5px;display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px;font-size:9px;font-weight:700;background:#fffbeb;border:1px solid #fcd34d;color:#92400e;">📈 ${sumber === 'prediksi' ? 'Prediksi OLS' : 'Aktual + Prediksi OLS'}</div>`
    : '';
...
function buildGeoProps_ISKA(hasilAnalisis, kategoriTerpilih, provinsiDipilih, getWarna, getKategori) {
  return {
    ...
    onEachFeature: (fitur, lapisan) => {
      const a   = fitur.properties?.iska_analysis || {};
      const w   = getWarna(fitur);
      lapisan.bindTooltip(buildTooltipHTML_ISKA(a, w), { sticky: true, opacity: 1, className: 'leaflet-tooltip-iska' });
...
const ActionBtn = ({ children, variant = 'primary', disabled, onClick }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={cn('flex items-center gap-1.5 px-3 py-2 rounded-xl font-bold text-xs transition-all active:scale-95 disabled:opacity-50 shadow-lg uppercase tracking-wider', v[variant])}>
      {children}
    </button>
  );
};

// ─── PETA ISKA (main export) ──────────────────────────────────────────────────
export default function PetaISKA({
  ...
  const geoProps = hasilAnalisis?.matched_features?.features
    ? buildGeoProps_ISKA(hasilAnalisis, kategoriTerpilih, provinsiDipilih, getWarna, getKategori)
    : null;

  const getButtonText = () => {
    if (sedangMenganalisis) return 'Memproses...';
    if (sedangCekData)      return 'Memeriksa Data...';
    if (!pernahAnalisis)    return 'Analisis ISKA';
    return ({ ALL:'ISKA Komposit', PERTANIAN:'D1 Pertanian', HORTIKULTURA:'D2 Hortikultura',
      KEHUTANAN:'D3 Kehutanan', HASIL_HUTAN:'D4 Hasil Hutan', PERKEBUNAN:'D5 Perkebunan',
      PERIKANAN:'D6 Perikanan', PERTAMBANGAN:'D7 Tambang' })[indikatorTerpilih] || 'Analisis';
  };
  ...
  const renderMapContent = (keyPrefix) => (
    <>
      ...
      <style>{`
        .leaflet-tooltip-iska { background:white !important; border:1px solid #e2e8f0 !important; border-radius:12px !important; box-shadow:0 8px 24px rgba(0,0,0,0.12) !important; padding:0 !important; }
        ...
        {adaPrediksi && (
          <div className="bg-white/97 dark:bg-slate-800/97 p-2.5 rounded-xl shadow-lg border backdrop-blur-sm max-w-[175px]"
            ...
            <p className="text-[9px] text-amber-600 dark:text-amber-400 leading-relaxed">
              Sebagian dimensi menggunakan prediksi Regresi Linear OLS karena tidak ada di aktual BPS
            </p>
          </div>
        )}
      ...
      <div className="relative" style={{ height: 480 }}>
        ...
        <div className="absolute bottom-4 left-4 z-[400]">
          <button onClick={() => setIsFullscreen(true)}
            className="flex items-center gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 border-2 border-slate-300 rounded-xl shadow-lg hover:bg-emerald-50 hover:border-emerald-500 transition-all active:scale-95 text-slate-800 dark:text-white font-semibold">
