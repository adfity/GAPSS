...
// Tab Metodologi: sama persis dengan versi admin
// Tab Tren      : baca-saja
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  ...
  Plus, Pencil, Eye, ChevronUp, ChevronRight, CheckCircle2,
  Search, AlertTriangle, Send, Clock, CheckCheck, XCircle,
  MessageSquarePlus, RefreshCw, Layers, TrendingDown,
} from 'lucide-react';
...
const DIMENSI_COLS = [
  { key: 's_padi',         label: 'D1\nPertanian',    rawKey: 'padi' },
  { key: 's_hortikultura', label: 'D2\nHortikultura', rawKey: 'hortikultura' },
  { key: 's_ttplahan',     label: 'D3\nKehutanan',    rawKey: 'ttplahan' },
  { key: 's_hahutan',      label: 'D4\nHsl Hutan',    rawKey: 'hahutan' },
  { key: 's_kebun',        label: 'D5\nPerkebunan',   rawKey: 'kebun' },
  { key: 's_ikan',         label: 'D6\nPerikanan',    rawKey: 'ikan' },
  { key: 's_proktam',      label: 'D7\nTambang',      rawKey: 'proktam' },
];
...
const EMPTY_USULAN = {
  status_kebijakan: 'SEDANG', prioritas: 3, pilar_kebijakan: 'Pertanian & Pangan',
  dimensi: 'S1_Pertanian', isu_strategis: '', kebijakan: '', rekomendasi_program: '',
  indikator_terkait: 'padi', catatan_user: '',
};
    ...
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          <Field label="Indikator Terkait" name="indikator_terkait" type="select"
            options={Object.keys(DATASET_LABELS_ISKA).map(k => ({ value: k, label: DATASET_LABELS_ISKA[k] }))} />
...
function ModalUsulanProvinsi({ provinsiNama, popupData, popupFitur, getWarna, getKategori,
  analysisId, onClose, onKirim, sending }) {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-3">
          {rekLocal.length === 0 ? (
            <div className="text-center py-8 text-slate-400">
              ...
              <p className="text-sm">Belum ada rekomendasi untuk diusulkan perubahan.</p>
            </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700 space-y-3 flex-shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => onKirim({ tipe: 'PROVINSI', analysis_id: analysisId, nama_provinsi: provinsiNama, rekomendasi_provinsi: rekLocal, catatan_user: catatan })}
              disabled={sending}
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="p-3.5 rounded-xl border-2 border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300 mt-0.5 leading-relaxed">
              Sebagian data dimensi tidak tersedia di database aktual BPS sehingga digantikan proyeksi model Regresi Linear OLS. Bukan data resmi BPS.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: 'Teranalisis',    val: hasilAnalisis.total_success || 0,     cls: 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200', valCls: 'text-emerald-700 dark:text-emerald-300' },
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button onClick={() => setMenuUnduh(!menuUnduh)}
                className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold text-sm shadow-sm">
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-600">
                  ...
                  <tr key={d.nama_provinsi} className={cn('hover:bg-emerald-50/40 dark:hover:bg-emerald-900/10 transition-colors', rowBg)}>
                    ...
                    <td className="px-4 py-3 text-center">
                      {isTA ? <span className="text-xs text-slate-400 italic">—</span>
                        ...
                        : <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200"><CheckCircle2 size={9} /> Aktual</span>}
                    </td>
                  </tr>
                );
              })}
          ...
          {adaProyeksi && (
            <div className="px-4 py-2 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-600">
              <p className="text-[10px] text-amber-600 flex items-center gap-1"><AlertTriangle size={10} /> * = data proyeksi Regresi Linear OLS (bukan data resmi BPS)</p>
            </div>
          )}
        </div>
      </div>
...
// ─── TAB KEBIJAKAN USER ───────────────────────────────────────────────────────
function TabKebijakanUser({ hasilAnalisis, kategoriTerpilih, setKategoriTerpilih,
  ...
  const muatUsulan = useCallback(() => {
    setUsulLoading(true);
    axios.get(`${API_BASE}/iska-usulan/saya/`)
  ...
  const filteredBank = useMemo(() => {
    ...
    return d;
  }, [bankData, fStatus, fPilar, fDimensi, searchBank]);
  ...
  const dataTerfilter = useMemo(() => {
    ...
    return f;
  }, [hasilAnalisis, kategoriTerpilih, searchProv, getKategori]);

  const popupFitur = modalProv ? (hasilAnalisis?.matched_features?.features || []).find(f => f.properties?.iska_analysis?.nama_provinsi === modalProv) : null;
  const popupData  = popupFitur?.properties?.iska_analysis;

  const handleKirimUsulan = async (payload) => {
    setSending(true);
    try {
      await axios.post(`${API_BASE}/iska-usulan/kirim/`, payload);
      setModalUsul(null); setModalProv(null);
  ...
  return (
    <div className="space-y-4">
      {modalUsul && <ModalUsulanKebijakan mode={modalUsul.mode} kebijakanLama={modalUsul.data} onClose={() => setModalUsul(null)} onKirim={handleKirimUsulan} sending={sending} />}
      {modalProv && popupData && <ModalUsulanProvinsi provinsiNama={modalProv} popupData={popupData} popupFitur={popupFitur} getWarna={getWarna} getKategori={getKategori} analysisId={analysisId} onClose={() => setModalProv(null)} onKirim={handleKirimUsulan} sending={sending} />}

      {toastMsg && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] px-4 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold shadow-2xl">{toastMsg}</div>
      )}
        ...
        {[
          ...
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={cn('flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-all',
              subTab === t.id ? 'bg-white dark:bg-slate-700 shadow text-emerald-600 dark:text-emerald-400'
            ...
            : (
              <>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <select value={fDimensi} onChange={e => setFD(e.target.value)}
                    className="text-sm px-3 py-2 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-200 outline-none cursor-pointer">
                ...
                {filteredBank.length === 0
                  ? <div className="py-10 text-center text-slate-400 text-sm">Tidak ada kebijakan cocok</div>
                  : (
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-slate-100 dark:bg-slate-700 sticky top-0">
                          <tr className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                            ...
                            <th className="px-3 py-3 text-left w-24">Dimensi</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                          {filteredBank.map((item, i) => {
                            const sc  = STATUS_COLORS[item.status?.replace(' ', '_')];
                            ...
                            return (
                              <React.Fragment key={item.id}>
                                <tr className={cn('cursor-pointer transition-colors',
                                  ...
                                  isExp && 'bg-emerald-50/50 dark:bg-emerald-900/10', 'hover:bg-slate-50 dark:hover:bg-slate-700/40')}
                                  ...
                                  <td className="px-3 py-2.5">
                                    <div className="flex flex-col gap-1">
                                      ...
                                      <span className="text-xs font-bold text-slate-500">P{item.prioritas}</span>
                                    </div>
                                  </td>
                                  ...
                                  <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                                    <button onClick={() => setModalUsul({ mode: 'edit', data: { id: item.id, status: item.status, prioritas: item.prioritas, pilar: item.pilar, dimensi: item.dimensi, isu_strategis: item.isu_strategis || '', kebijakan: item.kebijakan, rekomendasi: item.rekomendasi, indikator_terkait: item.indikator_terkait } })}
                                ...
                                {isExp && (
                                  <tr className="bg-emerald-50/30 dark:bg-emerald-900/10 border-b border-emerald-100 dark:border-emerald-800/30">
                                    <td colSpan={5} className="px-4 py-3">
                                      <div className="flex items-start gap-2.5">
                                        <ChevronRight size={13} className="text-emerald-400 flex-shrink-0 mt-0.5" />
                                        <div>
                                          <p className="text-xs font-bold text-emerald-600 mb-1">Rekomendasi Program:</p>
                                          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{item.rekomendasi}</p>
                                          {item.dasar_hukum && <p className="text-xs text-slate-400 mt-1">📋 Dasar Hukum: {item.dasar_hukum}</p>}
                                        </div>
                                      </div>
                                    </td>
                                  </tr>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="relative flex-1 min-w-[160px]">
                    ...
                    <input type="text" value={searchProv} onChange={e => setSP(e.target.value)} placeholder="Cari provinsi..."
                      className="w-full pl-9 pr-8 py-2 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm outline-none focus:border-emerald-400 text-slate-800 dark:text-slate-100" />
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-600 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-100 dark:bg-slate-700">
                      <tr className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                        <th className="px-4 py-3 text-left">Provinsi</th>
                        <th className="px-3 py-3 text-center">ISKA</th>
...
function TabMetodologi() {
  return (
    <div className="space-y-3">
        ...
        <div className="space-y-5">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Langkah 1 — Normalisasi Min-Max (lintas 38 provinsi)</p>
            </div>
            <MathBlock className="border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20">
              ...
              {' / '}
              <span>X<sub>max</sub> − X<sub>min</sub></span>{' × 100'}
            </MathBlock>
            <p className="text-xs text-slate-500 mt-2">Hasil Sᵢ berada di rentang <strong>0–100</strong>. Nilai 0 = produksi terendah; 100 = tertinggi.</p>
          </div>
          <div>
            <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                { d: 'D1', label: 'Pertanian',    col: 'padi',         satuan: 'Ku/Ha',     bg: '#84cc16' },
                { d: 'D2', label: 'Hortikultura', col: 'hortikultura', satuan: 'Kwintal',   bg: '#22c55e' },
                { d: 'D3', label: 'Kehutanan',    col: 'ttplahan',     satuan: 'Ha',        bg: '#14b8a6' },
                { d: 'D4', label: 'Hasil Hutan',  col: 'hahutan',      satuan: 'M³',        bg: '#16a34a' },
                { d: 'D5', label: 'Perkebunan',   col: 'kebun',        satuan: 'Ton',       bg: '#65a30d' },
                { d: 'D6', label: 'Perikanan',    col: 'ikan',         satuan: 'Ton',       bg: '#0891b2' },
                { d: 'D7', label: 'Pertambangan', col: 'proktam',      satuan: 'Miliar Rp', bg: '#f59e0b' },
              ].map(dim => (
                <div key={dim.d} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/40">
      ...
      <MetSection accentColor="#10b981" title="Klasifikasi Status" sub="4 kelas berdasarkan rentang skor ISKA" defaultOpen>
      ...
      <MetSection accentColor="#f59e0b" title="Catatan Proyeksi Data — Regresi Linear OLS" sub="Digunakan sebagai fallback jika data aktual tidak tersedia">
        <div className="space-y-3">
          <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/40 flex items-start gap-3">
            <TrendingDown size={18} className="text-emerald-500 flex-shrink-0 mt-0.5" />
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Regresi Linear OLS menarik garis lurus paling optimal di antara data historis, lalu memperpanjangnya ke masa depan untuk estimasi tren.
              Model dilatih pada data 2010–2025 (16 titik per provinsi).
            </p>
          </div>
          <div className="p-4 rounded-xl border-2 border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20">
            <div className="flex items-start gap-3">
              <AlertTriangle size={16} className="text-amber-500 flex-shrink-0 mt-0.5" />
              <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                Proyeksi merupakan <strong>estimasi matematis murni</strong> berdasarkan tren masa lalu (<em>ceteris paribus</em>). Model tidak memperhitungkan
                bencana alam, perubahan kebijakan, guncangan harga komoditas, atau lompatan teknologi. Bukan data resmi BPS.
              </p>
            </div>
          </div>
        </div>
...
function TabTren({ daftarTersimpan }) {
  ...
  if (!daftarTersimpan.length) return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <TrendingUp size={26} className="text-emerald-300 mx-auto mb-3" />
    ...
    <div className="space-y-5">
        ...
        {Object.keys(INDIKATOR_LABELS_ISKA).map(ind => {
          ...
          return (
            <button key={ind} onClick={() => ada && setFI(ind)}
              className={cn('px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1',
      ...
      {chartData.length === 0
        ...
        : (
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-600">
            <div className="text-sm font-bold text-slate-900 dark:text-white mb-3">{INDIKATOR_LABELS_ISKA[filterInd]} · {chartData.length} titik</div>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-600">
        ...
        <div className="flex flex-wrap gap-1.5">
          {TAHUN_TERSEDIA_ISKA.map(thn => {
            const ada = tahunCovered.includes(thn);
            const isProyeksi = thn > 2025;
            return (
...
// ─── MAIN EXPORT ──────────────────────────────────────────────────────────────
export default function TabsISKA_User({
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-600 shadow-sm overflow-hidden">
          ...
          return (
            <button key={id} onClick={() => setActiveTab(id)}
              className={cn('flex items-center justify-center gap-2 px-5 py-4 text-sm font-semibold transition-all relative flex-1 whitespace-nowrap',
                active ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-900/20'
      ...
      <div className="p-5">
        {activeTab === 'info'      && <TabInfo hasilAnalisis={hasilAnalisis} jumlahKategori={jumlahKategori} kategoriTerpilih={kategoriTerpilih} setKategoriTerpilih={setKategoriTerpilih} eksporData={eksporData} getWarna={getWarna} getKategori={getKategori} />}
        {activeTab === 'kebijakan' && <TabKebijakanUser hasilAnalisis={hasilAnalisis} kategoriTerpilih={kategoriTerpilih} setKategoriTerpilih={setKategoriTerpilih} getWarna={getWarna} getKategori={getKategori} analysisId={analysisId} />}
