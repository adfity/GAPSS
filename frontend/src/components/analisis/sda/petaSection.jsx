...
// ─── Status SDA (4 level, tema biru/teal)
const STATUS_SDA = {
...
// Label dataset SDA untuk modal cek
...
// ─── Shared helpers (juga di-export untuk dipakai tab)
...
export const Btn = ({ children, variant = 'primary', className = '', ...props }) => {
  ...
  return (
    <button
      className={cn('flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all active:scale-95 disabled:opacity-50', v[variant], className)}
      {...props}>
      {children}
    </button>
  );
...
export const AIBadgeSDA = ({ version, scores }) => (
  <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-700">
    ...
    <div className="flex-1 min-w-0">
      <div className="text-xs font-bold text-purple-700 dark:text-purple-300">
        Data Prediksi AI — Random Forest {version || 'sda_rf_v1.0'}
      </div>
      <div className="text-[10px] mt-0.5 text-purple-500 dark:text-purple-400">
        IPSDA dihitung dari model ML yang dilatih data historis BPS 2018–2024.
        {scores?.ipsda && (
          <span className="ml-1 font-semibold">
            CV R²={scores.ipsda.cv_r2?.toFixed(3)} · MAE={scores.ipsda.cv_mae?.toFixed(4)}
          </span>
        )}
      </div>
    </div>
...
const MapBtn = ({ children, onClick, className = '' }) => (
  <button
    onClick={onClick}
    className={cn('w-9 h-9 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl flex items-center justify-center shadow-md hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors', className)}>
    {children}
  </button>
);
...
// ─── Modal cek data BPS SDA
export function ModalCekDataSDA({ tahun, hasilCek, sedangCek, onTutup, onLanjutBPS, onLanjutAI }) {
  if (!hasilCek && !sedangCek) return null;

  const {
    semua_kosong, ada_yang_kosong, dataset_status, bisa_dieksekusi,
    ai_tersedia, ai_model_ready, ai_model_version, kosong,
  } = hasilCek || {};
  ...
  const iconCls = sedangCek ? 'text-blue-500 animate-spin' : hasAlert ? 'text-amber-500' : 'text-teal-500';

  const title = sedangCek
    ? `Memeriksa Data BPS SDA Tahun ${tahun}...`
    : semua_kosong
      ? `Data BPS SDA Tahun ${tahun} Tidak Tersedia`
      : ada_yang_kosong
        ? `Sebagian Data BPS SDA Tahun ${tahun} Tidak Tersedia`
        : `Data BPS SDA Tahun ${tahun} Siap`;
      ...
      <Card className="w-full max-w-lg overflow-hidden">
        ...
        <div className={cn('px-6 py-4 flex items-center gap-3 border-b', headerCls)}>
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">Pengecekan dataset BPS untuk IPSDA</div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 space-y-2">
          {sedangCek
            ? Object.keys(DATASET_LABELS_SDA).map(k => (
                <div key={k} className="flex items-center gap-3 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  <Loader2 size={12} className="text-blue-400 animate-spin" />
                  <span className="text-xs text-slate-500 dark:text-slate-400">{DATASET_LABELS_SDA[k]}</span>
                </div>
              ))
            : dataset_status && Object.entries(dataset_status).map(([k, info]) => (
                <div key={k} className={cn(
                  ...
                  <span className="text-xs font-medium text-slate-800 dark:text-slate-200 flex-1">
                    {DATASET_LABELS_SDA[k] || info.nama}
                  </span>
                  <span className={cn(
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-amber-700 dark:text-amber-300 mb-1">
                  {semua_kosong
                    ...
                    : `Dataset ${kosong?.map(k => DATASET_LABELS_SDA[k] || k).join(', ')} tidak tersedia di BPS tahun ${tahun}.`}
                </p>
                <p className="text-[11px] text-amber-600 dark:text-amber-400 leading-relaxed">
                  Pilih <strong>tahun lain</strong> yang datanya lengkap, atau gunakan{' '}
                ...
                {!ai_model_ready && (
                  <p className="text-[11px] text-red-500 mt-1 font-semibold">
                    ⚠ Model AI tidak ditemukan — pastikan file .pkl ada di folder ai_models/sda/
                  </p>
                )}
              </div>
            </div>
...
function MapEventHandler({ setKoordinat }) {
  ...
  const Inner = () => {
    useMapEvents({ mousemove: e => setKoordinat({ lat: e.latlng.lat.toFixed(4), lng: e.latlng.lng.toFixed(4) }) });
    return null;
  };
  return <Inner />;
}

// ─── PetaSDA — komponen utama peta
export default function PetaSDA({
  ...
  // ── Tooltip & Popup HTML untuk peta ──────────────────────────────────────
  const buildTooltip = (a) => {
    const c     = a.warna || '#cbd5e1';
    const aiTag = a.is_prediction ? ' 🤖' : '';
    return `
      <div style="font-family:inherit;padding:3px;min-width:150px;">
        ...
        <div style="font-size:8px;color:#64748b;margin-top:1px;">IPSDA: <strong>${a.ipsda ?? '-'}</strong></div>
      </div>`;
  };
  ...
  const buildPopup = (a) => {
    ...
    const aiTag = a.is_prediction ? ' 🤖' : '';
    const rows = [
      ['RPP Ikan',  d.rpp_ikan  != null ? Number(d.rpp_ikan).toFixed(6)  : '-'],
      ['RPP Kebun', d.rpp_kebun != null ? Number(d.rpp_kebun).toFixed(6) : '-'],
      ['NPI (Rp/jiwa)', d.npi   != null ? Number(d.npi).toLocaleString('id-ID', {maximumFractionDigits:0}) : '-'],
      ['KPS',       d.kps       != null ? Number(d.kps).toFixed(4)        : '-'],
    ];
    return `
      <div style="font-family:inherit;min-width:240px;">
        <div style="background:${c};color:white;padding:8px 10px;border-radius:8px 8px 0 0;margin:-1px -1px 8px -1px;">
          ...
          <div style="font-size:10px;font-weight:700;opacity:0.9;">${a.status || '-'} · IPSDA: ${a.ipsda ?? '-'}</div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;padding:0 4px 4px;">
          ${rows.map(([lbl, val]) => `
            <div style="background:#f8fafc;padding:5px;border-radius:6px;border-left:2px solid ${c};">
              ...
              <div style="font-size:10px;font-weight:800;color:#1e293b;">${val}</div>
            </div>`).join('')}
        </div>
  ...
  return (
    <Card className={cn('overflow-hidden', isAI ? 'border-2 border-purple-300 dark:border-purple-700' : 'border-2 border-blue-200 dark:border-blue-800')}>
      {/* AI banner */}
      {isAI && (
        <div className="flex items-center gap-2 px-4 py-2 bg-purple-50 dark:bg-purple-900/20 border-b border-purple-200 dark:border-purple-700">
          ...
          <span className="text-xs font-semibold text-purple-700 dark:text-purple-300">
            Prediksi AI — {hasilAnalisis?.source}
          </span>
          <span className="ml-auto text-[10px] text-purple-400">
            CV R²={hasilAnalisis?.model_scores?.ipsda?.cv_r2?.toFixed(3)} · MAE={hasilAnalisis?.model_scores?.ipsda?.cv_mae?.toFixed(4)}
          </span>
        </div>
      )}
      ...
      <div className="relative" style={{ height: isAI ? 496 : 520 }}>
        ...
        {leafletReady && MapCont && (
          <MapCont center={[-2.5, 118]} zoom={5} className="h-full w-full" zoomControl={false} ref={petaRef}>
            ...
            {hasilAnalisis?.matched_features?.features && (
              <GeoComp
                key={`${hasilAnalisis.tahun}-${statusTerpilih}-${provinsiHL}`}
                data={{ type: 'FeatureCollection', features: hasilAnalisis.matched_features.features }}
                style={feat => {
                  const a   = feat.properties?.sda_analysis || {};
                  const vis = statusTerpilih === 'SEMUA' || a.status === statusTerpilih;
                  const hl  = provinsiHL === a.nama_provinsi;
                  return {
                    fillColor:   a.warna || '#cbd5e1',
                    weight:      hl ? 3 : 1.5,
                    opacity:     vis ? 1 : 0,
                    color:       hl ? '#fff' : 'rgba(255,255,255,0.6)',
                    fillOpacity: vis ? 0.82 : 0,
                  };
                }}
                onEachFeature={(feat, layer) => {
                  const a = feat.properties?.sda_analysis || {};
                  layer.bindTooltip(buildTooltip(a), { sticky: true, opacity: 0.96 });
                  layer.bindPopup(buildPopup(a), { maxWidth: 270 });
                  layer.on('click', () => setProvinsiHL(a.nama_provinsi));
                }}
              />
            )}
          </MapCont>
              ...
              <div className="absolute left-full ml-2 top-0 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-xl z-[500] border border-slate-200 dark:border-slate-700 py-1">
                ...
                {Object.entries(BASEMAPS).map(([k, bm]) => (
                  <button key={k} onClick={() => { setBasemap(k); setShowBasemap(false); }}
                    className={cn('w-full text-left px-3 py-2 text-xs flex items-center justify-between transition-colors',
                      basemap === k ? 'text-blue-600 dark:text-blue-400 font-semibold' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'
                    )}>
              ...
              {searchOpen ? (
                <div className="absolute left-full ml-2 top-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 w-56">
                  <div className="p-2 flex gap-1.5">
                    ...
                    />
                    <button onClick={() => handleSearch()} className="bg-blue-600 hover:bg-blue-700 text-white px-2 rounded-lg transition-colors">
                      <Search size={12} />
                    </button>
                    <button onClick={() => { setSearchOpen(false); setSearchQuery(''); setProvinsiHL(null); }} className="bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 px-2 rounded-lg hover:bg-slate-300 transition-colors">
                      <X size={12} />
                    </button>
                  </div>
                ...
                <MapBtn onClick={() => setSearchOpen(true)}>
                  <Search size={13} className="text-slate-600 dark:text-slate-300" />
                </MapBtn>
              )}
            </div>
          )}
        ...
        {/* ── Tengah atas: Navigator Tahun ── */}
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[400]">
          <div className="relative">
            <button
              onClick={() => setShowTahunNav(!showTahunNav)}
              ...
              <div className="absolute top-full mt-1 left-1/2 -translate-x-1/2 w-36 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 z-[500] overflow-hidden py-1">
                {(TAHUN_TERSEDIA || []).slice().reverse().map(th => {
                  ...
                  return (
                    <button key={th}
                      onClick={() => {
                        setShowTahunNav(false);
                        hasSaved ? onMuatTahun(savedBPS[0].analysis_id, savedBPS[0].tahun) : onPilihTahunDenganTahun(th);
                      }}
                      className={cn(
                        'w-full flex items-center justify-between px-4 py-2 text-sm transition-colors',
                        isActive ? 'bg-slate-100 dark:bg-slate-700 font-bold text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'
                      )}>
                      <span>{th}</span>
                      {hasSaved && <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />}
                    </button>
                  );
                })}
              </div>
        ...
        {/* ── Kanan atas: Koordinat + Legenda IPSDA ── */}
        <div className="absolute top-3 right-3 z-[400] flex flex-col gap-2 items-end">
          ...
          <div className="bg-white/95 dark:bg-slate-800/90 px-3 py-1.5 rounded-lg shadow border border-slate-200 dark:border-slate-600">
            <div className="text-[10px] font-mono text-slate-600 dark:text-slate-300 whitespace-nowrap">
              <span className="text-blue-600 dark:text-blue-400 font-bold">Lat:</span> {koordinat.lat} |{' '}
              <span className="text-blue-600 dark:text-blue-400 font-bold">Lng:</span> {koordinat.lng}
            </div>
          </div>

          {/* Legenda + filter */}
          <div className="bg-white/95 dark:bg-slate-800/90 p-3 rounded-xl shadow-lg border border-slate-200 dark:border-slate-600 min-w-[115px]">
            <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-2">Status IPSDA</div>
            {Object.entries(STATUS_SDA).map(([k, v]) => (
            ...
            <div className="relative mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
              ...
              {menuFilter && (
                <div className="absolute bottom-full mb-1 right-0 w-full bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-[500] py-1">
                  {['SEMUA', 'OPTIMAL', 'CUKUP', 'KURANG', 'RENDAH'].map(s => (
                    <button key={s} onClick={() => { setStatusTerpilih(s); setMenuFilter(false); }}
                      className={cn(
                        'w-full text-left px-3 py-1.5 text-[10px] font-semibold transition-colors hover:bg-slate-50 dark:hover:bg-slate-700',
                        statusTerpilih === s ? 'text-blue-600 dark:text-blue-400' : 'text-slate-700 dark:text-slate-300'
                      )}>
                      {s}
                    </button>
                  ))}
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[400] flex gap-2">
          <Btn
            onClick={onAnalisis} disabled={sedangMenganalisis || sedangCekData} variant="primary"
            ...
            {sedangCekData ? 'Memeriksa...' : sedangMenganalisis ? 'Memproses...' : 'Analisis SDA'}
          </Btn>
          {dataBaruDariBPS && hasilAnalisis && (
            <Btn variant="save" onClick={onSimpan} className="px-5 uppercase tracking-wider text-xs shadow-xl shadow-teal-600/30">
              <Save size={13} /> Simpan
            </Btn>
          )}
