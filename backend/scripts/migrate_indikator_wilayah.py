...
DDL = """
CREATE TABLE IF NOT EXISTS indikator_wilayah (
    id              SERIAL PRIMARY KEY,
    kode_wilayah    INTEGER       NOT NULL,
    wilayah         VARCHAR(150)  NOT NULL,
    provinsi        VARCHAR(150)  NOT NULL,
    level           VARCHAR(20)   NOT NULL,   -- 'provinsi' atau 'kab_kota'
    tahun           SMALLINT      NOT NULL,   -- 2020-2025

    -- ISDM
    hls             NUMERIC(8,4),   -- Harapan Lama Sekolah
    rls             NUMERIC(8,4),   -- Rata-rata Lama Sekolah
    ahh             NUMERIC(8,4),   -- Angka Harapan Hidup
    pengeluaran     NUMERIC(12,4),  -- Pengeluaran Per Kapita
...
INDEXES = [
    "CREATE INDEX IF NOT EXISTS idx_iw_kode     ON indikator_wilayah (kode_wilayah);",
    "CREATE INDEX IF NOT EXISTS idx_iw_tahun    ON indikator_wilayah (tahun);",
    "CREATE INDEX IF NOT EXISTS idx_iw_level    ON indikator_wilayah (level);",
    "CREATE INDEX IF NOT EXISTS idx_iw_provinsi ON indikator_wilayah (provinsi);",
]

# Semua kolom indikator (urutan harus sama dengan INSERT di bawah)
INDIKATOR_COLS = [
    'hls', 'rls', 'ahh', 'pengeluaran',
    'hortikultura', 'padi', 'ttplahan', 'hahutan', 'kebun', 'ikan', 'proktam',
    'komposit', 'ketersediaan', 'keterjangkauan', 'pemanfaatan',
    'spk', 'spe',
]
...
def migrate_indikator_wilayah():
    print("\n" + "=" * 60)
    print("  MIGRATE INDIKATOR WILAYAH (PostgreSQL)")
    print("=" * 60)
    ...
    # Dedup — semua duplikat identik, ambil baris pertama
    before = len(df)
    ...
    try:
        ...
        cur.execute(DDL)
        conn.commit()
        print(f"  Tabel 'indikator_wilayah' siap")

        # Hapus data lama
        cur.execute("SELECT COUNT(*) FROM indikator_wilayah;")
        existing = cur.fetchone()[0]
        cur.execute("TRUNCATE TABLE indikator_wilayah RESTART IDENTITY;")
        conn.commit()
        ...
        conn.commit()
        print(f"  BERHASIL: {len(rows):,} baris -> tabel 'indikator_wilayah'")
        ...
        cur.execute("SELECT MIN(tahun), MAX(tahun) FROM indikator_wilayah;")
        tahun_range = cur.fetchone()

        cur.execute("SELECT level, COUNT(*) FROM indikator_wilayah GROUP BY level ORDER BY level;")
...
def main():
    print("\n" + "=" * 60)
    print("  MIGRATE INDIKATOR WILAYAH -- MULAI")
    print("=" * 60)

    migrate_indikator_wilayah()

    print("\n" + "=" * 60)
    print("  MIGRATE INDIKATOR WILAYAH -- SELESAI")
    print("=" * 60 + "\n")
