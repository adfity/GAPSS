...
# ── Normalisasi status per indeks ─────────────────────────────────────────────
#
# ISDM, IPE, ISKA  → 4 kelas: RENDAH, SEDANG, TINGGI, SANGAT_TINGGI
#   JSON pakai spasi → perlu diubah ke underscore
#
# IKP              → 6 kelas ketahanan pangan: SANGAT_RENTAN, RENTAN,
#                    AGAK_RENTAN, AGAK_TAHAN, TAHAN, SANGAT_TAHAN
#   JSON pakai spasi → perlu diubah ke underscore
#
# Semua status akhir di DB memakai UNDERSCORE (konsisten dengan kode analisis).
...
def _normalize_status(raw: str) -> str:
    """Ubah status dari JSON (spasi) ke format DB (underscore)."""
    raw = str(raw or '').strip().upper()
    return _STATUS_NORM_MAP.get(raw, raw.replace(' ', '_'))
...
def migrate_bank_kebijakan():
    ...
    # ── Cari file JSON ────────────────────────────────────────────────────────
    # Coba beberapa nama file (v2 bisa ganti nama)
    candidates = [
        'bank_kebijakan_revisi_v2_160.json',
        'bank_kebijakan_revisi_v2.json',
        'bank_kebijakan.json',
    ]
    file_path = None
    search_dirs = [
        os.path.abspath(os.path.join(base_dir, '..', '..', '..', 'frontend', 'public', 'data')),
        os.path.abspath(os.path.join(base_dir, '..', '..', '..', 'frontend', 'public', 'data', 'boundary')),
        base_dir,
    ]
    ...
    if not file_path:
        print(f"  ❌ File JSON tidak ditemukan. Coba letakkan salah satu dari:")
        for fname in candidates:
            print(f"       {os.path.join(search_dirs[0], fname)}")
        return
    ...
    try:
        ...
        # Tambahkan kolom baru jika tabel sudah ada (idempoten)
        for col, col_type in [("dasar_hukum", "TEXT"), ("dimensi", "VARCHAR(100)")]:
            cur.execute(f"""
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_name='bank_kebijakan' AND column_name='{col}'
                    ) THEN
                        ALTER TABLE bank_kebijakan ADD COLUMN {col} {col_type};
                    END IF;
                END$$;
            """)

        # ── Hapus data lama & reset sequence ──────────────────────────────────
        cur.execute("TRUNCATE TABLE bank_kebijakan RESTART IDENTITY;")
        print(f"  🗑️  Data lama dihapus (TRUNCATE)")

        # ── Insert dengan normalisasi ─────────────────────────────────────────
        inserted       = 0
        skipped        = 0
        counts_per_idx = {}   # ringkasan per indeks

        for i, row in enumerate(data):
            raw_indeks = row.get('indeks')
            raw_status = str(row.get('status', '')).strip()

            # ── Tentukan indeks ────────────────────────────────────────────────
            # Baris tanpa field 'indeks' (atau null) = ISDM
            if not raw_indeks:
                indeks = 'ISDM'
            else:
                indeks = str(raw_indeks).strip().upper()

            # ── Normalisasi status (spasi → underscore) ────────────────────────
            status = _normalize_status(raw_status)
            if not status:
                print(f"  ⚠️  Row {i}: status kosong, dilewati")
                skipped += 1
                continue

            # ── Validasi field wajib ───────────────────────────────────────────
            kebijakan   = str(row.get('kebijakan',           '') or '').strip()
            rekomendasi = str(row.get('rekomendasi_program', '') or '').strip()
            if not kebijakan or not rekomendasi:
                print(f"  ⚠️  Row {i}: kebijakan/rekomendasi kosong, dilewati")
                skipped += 1
                continue
            ...
            try:
                cur.execute("""
                    INSERT INTO bank_kebijakan
                        (indeks, dimensi, status, prioritas, pilar_kebijakan,
                         isu_strategis, kebijakan, rekomendasi_program,
                         dasar_hukum, indikator_terkait)
                ...
                """, (
                    ...
                    status,
                    row.get('prioritas'),
                    row.get('pilar_kebijakan'),
                    row.get('isu_strategis'),
                    kebijakan,
                    rekomendasi,
                    row.get('dasar_hukum'),
                    row.get('indikator_terkait'),
                ))
                inserted += 1
                counts_per_idx[indeks] = counts_per_idx.get(indeks, 0) + 1
        ...
        conn.commit()
        print(f"\n  ✅ BERHASIL: {inserted} data → tabel 'bank_kebijakan'")
        if skipped:
            print(f"  ⚠️  Dilewati: {skipped} baris")

        # ── Ringkasan per indeks ───────────────────────────────────────────────
        print(f"\n  ── Ringkasan per indeks ──────────────────────────────")
        for idx, cnt in sorted(counts_per_idx.items()):
            print(f"     • {idx:<10} : {cnt} baris")

        # ── Verifikasi akhir dari DB ───────────────────────────────────────────
        print(f"\n  ── Verifikasi DB ─────────────────────────────────────")
        cur.execute("""
            SELECT indeks, status, COUNT(*) as cnt
            FROM bank_kebijakan
            GROUP BY indeks, status
            ORDER BY indeks, status
        """)
        rows_check = cur.fetchall()
        current_idx = None
        for indeks, status, cnt in rows_check:
            if indeks != current_idx:
                print(f"\n     [{indeks}]")
                current_idx = indeks
